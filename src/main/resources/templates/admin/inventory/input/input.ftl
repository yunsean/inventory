...
<@html>
            ...
            <@panel>
                ...
                <@panelBody>
                    <div class="tableToolContainer" style="margin-bottom:15px">
                        <div class="form-inline" action="/inv/input">                            &nbsp; &nbsp;
                            ...
                            <div class="form-group">
                                ...
                                <div style="display: inline-block">
                                    <input class="form-control" id="goods-input" type="text" autocomplete="off" style="width: 150px;"/>
                            ...
                            <a href="#" class="btn btn-success" onclick="doSubmit()">
                                <i class="icon icon-eye-slash"></i>入库提交
                            </a>
                            <div style="float:right">
                                ...
                                <@shiro.hasPermission name="inventory_goods.edit" >
                                    <button onclick="doAddGoods()" class="btn btn-success ">
                                        <i class="icon icon-pencil "></i>添加资产
                                    </button>
                                </@shiro.hasPermission>
                            </div>
                        </div>
                    </div>
                    <iframe id="intent_list" width="100%" src="/admin/inventory/input/list" onload="Javascript:SetCwinHeight()" frameborder="0" src="index.htm"></iframe>
                </@panelBody>
            </@panel>
        </@bodyContent>
    </@bodyFrame>
    ...
    <script>
        ...
        $(function () {
            setupAutoCompleteGoods($("#goods-input"), $("#goods-result"), $("#goods-value"), "goods-table", addGoods2);
        });
    ...
    <script>
        ...
        function doFilter() {
            ...
            document.getElementById('intent_list').src = "/admin/inventory/input/list?keyword=" + filter;
        }
        function doAdd() {
            var count = $("#add_count").val();
            ...
            $("#add_barcode").val("");
            $.post(
                "/admin/inventory/input/add/code.json",
    ...
    <script>
        ...
        function batchAdd() {
            ...
            $.post(
                "/admin/inventory/input/add/batch.json",
                {barcodes: barcodes},
                function (data) {
                    if (data.code < 0) {
                        alertShow("danger", data.message, 3000);
        ...
        function doClean() {
            warningModal("确定要清空待入库清单吗?", function () {
                $.ajax({
                    url: "/admin/inventory/input/clean.json",
                    type: 'DELETE',
    ...
    <@modal title="入库提交" idPrefix="submit" onOk="submit" width=50>
    ...
    <script>
        ...
        function submit() {
            ...
            $.post(
                "/admin/inventory/input/submit.json",
                {remark: remark},
                function (data) {
                    if (data.code < 0) {
                        alertShow("danger", data.message, 3000);
