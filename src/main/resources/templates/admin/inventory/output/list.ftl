...
<@html>
    ...
    <div class="container-fluid">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr <#if item.goodsId?default(0) == 0>bgcolor="#fffff0"</#if>>
                    ...
                    <@td true>
                        <a onclick="addOutput('${(item.goodsId?c)!}', -1)">
        ...
        <@panelPageFooter action="/admin/inventory/output/list" />
    </div>

    <script>
        function addOutput(id, count) {
            $.post(
        ...
        function setNumber(id, output) {
            var count = $(output).val();
            $.post(
                    "/admin/inventory/output/add/set.json",
                    {id: id, count: count},
                    function (data) {
                        if (data.code < 0) {
                            alertShow("danger", data.message, 3000);
        ...
        function doDelete(id) {
            warningModal("确定要删除该分类吗?", function(){
                $.ajax({
                    url: "/admin/inventory/output/delete.json?id=" + id,
                    type: 'DELETE',
